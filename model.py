...
def get_odds_consensus(home_name, away_name):
    ...
    try:
        params = {
            ...
            'markets': 'h2h',
            'oddsFormat': 'decimal',
            'bookmakers': 'tipico_de,bet365,bwin,unibet,betway'
        }
        ...
        if r.status_code != 200:
            ...
            return None, None, None, None, []
        games = r.json()
        home_search = home_name.lower().replace(' ', '')
        away_search = away_name.lower().replace(' ', '')
        for game in games:
            gh = game.get('home_team', '').lower().replace(' ', '')
            ga = game.get('away_team', '').lower().replace(' ', '')
            if (home_search[:4] in gh or gh[:4] in home_search) and (away_search[:4] in ga or ga[:4] in away_search):
                home_odds_list = []
                away_odds_list = []
                tipico_home = None
                tipico_away = None
                ...
                for bookie in game.get('bookmakers', []):
                    ...
                    for market in bookie.get('markets', []):
                        if market.get('key') == 'h2h':
                            ...
                            h_odd = next((o['price'] for o in outcomes if o['name'].lower().replace(' ','') == gh[:6]), None)
                            a_odd = next((o['price'] for o in outcomes if o['name'].lower().replace(' ','') == ga[:6]), None)
                            if not h_odd:
                                h_odd = outcomes[0]['price'] if len(outcomes) > 0 else None
                ...
                if home_odds_list:
                    ...
                    t_home = tipico_home if tipico_home else avg_home_odd
                    t_away = tipico_away if tipico_away else avg_away_odd
                    print(f'    Odds gefunden: {len(bookies_used)} Bookies - Konsensus {consensus_home:.1%}/{consensus_away:.1%}')
                    return round(consensus_home, 3), round(consensus_away, 3), round(t_home, 2), round(t_away, 2), bookies_used
        print(f'    Keine Odds fuer {home_name} vs {away_name}')
        return None, None, None, None, []
    except Exception as e:
