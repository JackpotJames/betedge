...
# The Odds API Key - kostenlos unter https://the-odds-api.com
...
# Cache fuer Odds API - einmal laden, alle Spiele nutzen
_odds_cache = None
...
def load_all_odds():
    ...
    try:
        ...
        r = requests.get(ODDS_API_URL, params=params, headers=HEADERS, timeout=15)
        if r.status_code != 200:
            print(f'Odds API Fehler: {r.status_code}')
            _odds_cache = []
            return _odds_cache
        _odds_cache = r.json()
        print(f'Odds API: {len(_odds_cache)} Spiele geladen')
        return _odds_cache
    except Exception as e:
...
def get_odds_consensus(home_name, away_name):
    ...
    try:
        # Normalisiere Namen fuer Matching
        def norm(s):
            words = s.lower().split()
            # Entferne haeufige Woerter
            stop = ['the', 'golden', 'state', 'new', 'york', 'los', 'angeles', 'san', 'oklahoma', 'city', 'portland', 'trail']
            key_words = [w for w in words if w not in stop and len(w) > 2]
            return key_words

        home_words = norm(home_name)
        away_words = norm(away_name)

        best_game = None
        best_score = 0

        for game in games:
            gh_words = norm(game.get('home_team', ''))
            ga_words = norm(game.get('away_team', ''))

            h_match = sum(1 for w in home_words if any(w in gw or gw in w for gw in gh_words))
            a_match = sum(1 for w in away_words if any(w in gw or gw in w for gw in ga_words))
            score = h_match + a_match
        ...
        if not best_game or best_score < 1:
            print(f'    Kein Match fuer {home_name} vs {away_name}')
            return None, None, None, None, []
        ...
        for bookie in best_game.get('bookmakers', []):
            ...
            for market in bookie.get('markets', []):
                if market.get('key') == 'h2h':
                    ...
                    # Finde Home und Away Odds by team name
                    h_odd = None
                    a_odd = None
                    for o in outcomes:
                        oname = o['name'].lower()
                        if any(w in oname for w in norm(api_home)):
                            h_odd = o['price']
                        elif any(w in oname for w in norm(api_away)):
                            a_odd = o['price']
                    # Fallback: erste zwei Outcomes
                    if not h_odd and len(outcomes) >= 2:
                        h_odd = outcomes[0]['price']
                        a_odd = outcomes[1]['price']
                    if h_odd and a_odd and h_odd > 1 and a_odd > 1:
                        home_odds_list.append(h_odd)
                        away_odds_list.append(a_odd)
        ...
        avg_home_odd = sum(home_odds_list) / len(home_odds_list)
        avg_away_odd = sum(away_odds_list) / len(away_odds_list)
        raw_h = 1 / avg_home_odd
        raw_a = 1 / avg_away_odd
        total = raw_h + raw_a
        consensus_home = raw_h / total
        consensus_away = raw_a / total
        t_home = tipico_home if tipico_home else round(avg_home_odd, 2)
        t_away = tipico_away if tipico_away else round(avg_away_odd, 2)
        print(f'    {api_home} vs {api_away}: {len(bookies_used)} Bookies, Konsensus {consensus_home:.1%}/{consensus_away:.1%}, Tipico: {t_home}/{t_away}')
        return round(consensus_home, 3), round(consensus_away, 3), round(t_home, 2), round(t_away, 2), bookies_used
    except Exception as e:
        print(f'Odds Matching Fehler: {e}')
        return None, None, None, None, []
...
def main():
    ...
    for g in games:
        ...
        print(f'  Lade Bookie-Odds: {g["home_name"]} vs {g["away_name"]}...')
        cons_home, cons_away, tipico_h, tipico_a, bookies = get_odds_consensus(g['home_name'], g['away_name'])
        time.sleep(0.5)
        if cons_home and cons_away:
            book_home = cons_home
            book_away = cons_away
            home_odds = tipico_h if tipico_h else round((1 / cons_home) * 0.94, 2)
            away_odds = tipico_a if tipico_a else round((1 / cons_away) * 0.94, 2)
            odds_source = f'Konsensus ({len(bookies)} Bookies)'
        else:
            h_net = h_stats.get('pts_for', 110) - h_stats.get('pts_against', 110)
            a_net = a_stats.get('pts_for', 110) - a_stats.get('pts_against', 110)
            h_wp = h_stats.get('win_pct', 0.5)
            a_wp = a_stats.get('win_pct', 0.5)
            raw_book = 0.5 + (h_wp - a_wp) * 0.45 + ((h_net - a_net) / 25) * 0.30 + 0.035
            book_home = max(0.28, min(0.78, raw_book))
            book_away = 1 - book_home
            home_odds = round((1 / book_home) * 0.94, 2)
            away_odds = round((1 / book_away) * 0.94, 2)
            odds_source = 'Modell-Schätzung (kein API Key)'
        print(f'    Odds Quelle: {odds_source}')
        ...
        output_games.append({
            ...
            "prediction": {
                ...
                "value_bet": value_bet,
                "key_factors": factors,
                "tipico_note": f"Quoten via {odds_source}",
