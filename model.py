...
def get_game_stats(game_id):
    ...
    for t in boxscore.get('teams', []):
        team_abbr = normalize_abbr(t.get('team', {}).get('abbreviation', ''))
...
def get_team_advanced_stats(abbr, n=N_RECENT):
    ...
    for i, game in enumerate(games):
        ...
        for competitor in comp.get('competitors', []):
            comp_abbr = normalize_abbr(competitor.get('team', {}).get('abbreviation', ''))
            if comp_abbr == abbr:
...
# Global ESPN abbreviation normalizer — ESPN liefert manchmal 'NY' statt 'NYK' etc.
ESPN_ABBR_MAP = {
    'NY': 'NYK', 'GS': 'GSW', 'NO': 'NOP', 'SA': 'SAS',
    'UTAH': 'UTA', 'WSH': 'WAS'
}
def normalize_abbr(raw):
    return ESPN_ABBR_MAP.get(raw, raw)
    ...
    for offset in range(1, 5):
        ...
        for event in d.get('events', []):
            ...
            for t in comp.get('competitors', []):
                if normalize_abbr(t.get('team', {}).get('abbreviation', '')) == abbr:
                    return offset - 1
        time.sleep(0.2)
    return 3
        ...
        try:
            ...
            for event in d.get('events', []):
                ...
                if not home_t or not away_t:
                    continue
                if (normalize_abbr(home_t['team']['abbreviation']) == entry['home_abbr'] and
                    normalize_abbr(away_t['team']['abbreviation']) == entry['away_abbr']):
                    actual_winner = normalize_abbr(home_t['team']['abbreviation']) if home_t.get('winner') else normalize_abbr(away_t['team']['abbreviation'])
...
def main():
    ...
    for g in games:
        ha = normalize_abbr(g['home_abbr'])
        aa = normalize_abbr(g['away_abbr'])
        print(f"\n  {ha} vs {aa}:")
